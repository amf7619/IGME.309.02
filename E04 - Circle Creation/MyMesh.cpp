...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	...
	for (int i = 0; i < a_nSubdivisions; i++) 
	{
		...
		vector3 firstVertex = vector3(cos(constant * i) * a_fRadius, sin(constant * i) * a_fRadius, 0);
		vector3 secondVertex = vector3(cos(constant * (i + 1)) * a_fRadius, sin(constant * (i + 1)) * a_fRadius, 0);
		AddTri(origin, firstVertex, secondVertex);
	}
